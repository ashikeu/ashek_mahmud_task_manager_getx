part of 'app_pages.dart';
// DO NOT EDIT. This is code generated via package:get_cli/get_cli.dart

abstract class Routes {
  Routes._();
  static const HOME = _Paths.HOME;
  static const NEW_TASK_LIST = _Paths.NEW_TASK_LIST;
  static const PROGRESS_TASK_LIST = _Paths.PROGRESS_TASK_LIST;
  static const COMPLETED_TASK_LIST = _Paths.COMPLETED_TASK_LIST;
  static const CANCELED_TASK_LIST = _Paths.CANCELED_TASK_LIST;
  static const SIGN_IN_SCREEN = _Paths.SIGN_IN_SCREEN;
  static const SIGN_UP_SCREEN = _Paths.SIGN_UP_SCREEN;
  static const SPLASH_SCREEN = _Paths.SPLASH_SCREEN;
  static const FORGET_PASSWORD_VERIFY_EMAIL_SCREEN =_Paths.FORGET_PASSWORD_VERIFY_EMAIL_SCREEN;
  static const FORGET_PASSWORD_VERIFY_OTP_SCREEN =_Paths.FORGET_PASSWORD_VERIFY_OTP_SCREEN;
  static const RESET_PASSWORD_SCREEN = _Paths.RESET_PASSWORD_SCREEN;
}

abstract class _Paths {
  _Paths._();
  static const HOME = '/home';
  static const NEW_TASK_LIST = '/new-task-list';
  static const PROGRESS_TASK_LIST = '/progress-task-list';
  static const COMPLETED_TASK_LIST = '/completed-task-list';
  static const CANCELED_TASK_LIST = '/canceled-task-list';
  static const SIGN_IN_SCREEN = '/sign-in-screen';
  static const SIGN_UP_SCREEN = '/sign-up-screen';
  static const SPLASH_SCREEN = '/splash-screen';
  static const FORGET_PASSWORD_VERIFY_EMAIL_SCREEN =
      '/forget-password-verify-email-screen';
  static const FORGET_PASSWORD_VERIFY_OTP_SCREEN =
      '/forget-password-verify-otp-screen';
  static const RESET_PASSWORD_SCREEN = '/reset-password-screen';
}
